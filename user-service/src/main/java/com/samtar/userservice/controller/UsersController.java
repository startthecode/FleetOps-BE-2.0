...
public class UsersController {
    ...
    public ResponseEntity<SuccessApiResponse<SignInRespDto>> signIn(@RequestBody SignInReqDto req, HttpServletResponse response) {
        SignInRespDto responseData = userServices.signin(req);
        SuccessApiResponse<SignInRespDto> resp = new SuccessApiResponse<>(MessageConstant.USER_SIGNIN, responseData, LocalDateTime.now());
        response.addCookie(authCookieUtil.addAuthTokenCookie(responseData.refreshToken()));
        return ResponseEntity.ok(resp);
    }

    // signup
    public ResponseEntity<SuccessApiResponse<SignUpResDto>> signUp(@RequestBody SignUpReqDto req, HttpServletResponse response) {
        SignUpResDto responseData = userServices.signUp(req);
        SuccessApiResponse<SignUpResDto> resp = new SuccessApiResponse<>(MessageConstant.USER_CREATED, responseData, LocalDateTime.now());
        response.addCookie(authCookieUtil.addAuthTokenCookie(responseData.refreshToken()));
        return ResponseEntity.ok(resp);
    }
}
